...
* **Providing a better understanding of the left stick position in regards to boost energy behavior and charging time required** upon Fancy Flying. Maybe it can even help to train **[Flap Boost](https://journey.fandom.com/wiki/Flap_Boost)** !
...
The max boost energy graphics above (_Journey_-processed inputs) are designed for when the **camera aims fully down (52° angle)**, and for **aerial dives** only:
...
* Max boost energy colors would look very similar, if not the same. Because of it, upon using full stick pressure the colors act as both a **boost energy and speed meter indicator** (for aerial dives and dropshoots ! With lower max boost energy values giving more speed).
...
## Known limitations
* The Slow and Flap pressure range toggle is based on the use of a mask layer. This mask layer was created by measuring in-game with Cheat Engine the specific coordinates responsible for the wayfarer adjusting their body angle, or the maximum boost energy changing drastically. Not every coordinates were tried out, so the curve was partially interpolated. This means you should expect a 1 or 2 pixels error margin compared to the reality in _Journey_ (and even more with the "smooth" input mode).
